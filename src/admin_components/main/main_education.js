...
export default function Main_education() {
  ...
  useEffect(() => {
    ...
    axios.get(`${process.env.REACT_APP_SERVER_URL}/api/formdata`)
  ...
  const handleAddField = () => {
    setFormData([...formData, { api:'', rest_id: '', api_id: '', exam_name: '', price: '', enable: true }]);
  };

  const handleSaveFormData = (e) => {
    e.preventDefault();

    axios.post(`${process.env.REACT_APP_SERVER_URL}/api/formdata`, formData)
  ...
  return (
    ...
    <section className='flex-1 bg-white grid grid-cols-1 gap-0 '>
      <div className='mt-6 text-xl p-6 mx-auto rounded-2xl text-center text-green-600 font-mono font-extrabold mb-4 shadow-2xl'>Result Checker Settings</div>
        ...
        {formData.map((data, index) => (
          <div key={index} className='mb-4 font-mono md:mx-20 mx-4 md:p-16 p-10 rounded-3xl shadow-3xl flex flex-col justify-center items-center md:grid md:grid-cols-2 bg-slate-100 md: gap-x-8 '>
            ...
            <div className='mb-4 grid grid-cols-3'>
              ...
              <input value={data.exam_name} onChange={(e) => handleInputChange(index, 'exam_name', e.target.value)} name='exam_name' placeholder='Exam Name' type='text' className='placeholder:text-center text-center col-span-2 outline-none ml-2 rounded-xl h-14 appearance-none border border-gray-500' />
            </div>
            <div className='mb-4 grid grid-cols-3'>
              <label>Price per PIN</label>
              <input value={data.price} onChange={(e) => handleInputChange(index, 'price', e.target.value)} name='price' placeholder='price per PIN' type='text' className='placeholder:text-center text-center col-span-2 outline-none ml-2 rounded-xl h-14 appearance-none border border-gray-500' />
            </div>
            <div className='mb-4 grid grid-cols-3'>
              ...
              <div className='mb-4 grid grid-cols-3'>
                ...
                <label className='col-span-2'>Disenable this plan</label>
            ...
            <div className='h-32 flex justify-around items-center mb-6'>
              <button type='button' onClick={handleAddField} className='bg-blue-900 text-white h-14 p-4 rounded-2xl hover:bg-blue-600'>Add_Exam_plan</button>
