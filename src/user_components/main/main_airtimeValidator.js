import React from 'react'
import { useState, useEffect } from 'react'
import axios from 'axios'
import Swal from 'sweetalert2';
import { useNavigate } from 'react-router-dom';
export default function Main_data({ toggleValidator, details, details2 }) {

  const navigator = useNavigate();
  //check if there is session
  const [toggler, setToggler] = useState(false)

  const handleToggler = () => {
    setToggler(!toggler)

  }


  useEffect(() => {

    axios.get(`${process.env.REACT_APP_SERVER_URL}/api/getrequestuser/user_session`, { withCredentials: true })
      .then(response => {
        const mydata = response.data
        if (!mydata.session) {
          navigator("/")
        }

      })

  }, [])




  const showAlert = (status, message, icon) => {
    Swal.fire({
      title: status,
      text: message,
      icon: icon,
      confirmButtonText: 'OK'
    });
  }

  const amount = parseInt(details.amount);
  const percentage = parseInt(details.percentage);
  const amount2Pay = amount - amount * percentage / 100


  const handleEdit = () => {
    toggleValidator(false);

  }
  const handleSubmit = () => {

    handleToggler();


    const fullData = Object.assign({}, details, details2);
    //console.log(fullData)
    axios.post(`${process.env.REACT_APP_SERVER_URL}/api/airtime`, fullData, { withCredentials: true })
      .then(result => {
        const data = result.data
        if (data.status === "success" || data.status === "process") {

          showAlert(data.status, data.message, "success")
        } else {
          showAlert(data.status, data.message, "error")

        }
        setToggler(false)

      })
      //.then(showAlert())
      .catch(err => console.error(err))

  }

  return (

    <section className='flex flex-1 justify-center  shadow-2xl h-screen items-center rounded-2xl '>

      <div className='bg-white shadow-2xl w-4/5 md:w-2/6 flex justify-center flex-col rounded-xl text-black mt-10 '>
        <div className='text-center bg-green-400 h-10 rounded text-white font-mono text-xl'>Details Confirmation!</div>
        <div className='mt-5 w-full flex justify-center'>
          <div className='text-md font-bold font-mono italic'>Network: {details.airtime_name}</div>
        </div>
        <div className='mt-1 w-full flex justify-center'>
          <div className=' justify-start text-md font-bold font-mono italic'>Amount: N{details.amount}</div>
        </div>
        <div className='mt-1 w-full flex justify-center'>
          <div className='text-md font-bold font-mono italic'>Phone: {details.phone}</div>
        </div>
        <div className='mt-5 w-full flex justify-center  mb-1'>
          <button onClick={handleEdit} type='button' className='text-center bg-red-700 rounded-2xl h-12 w-3/5 hover:bg-red-500 text-white'>Edit</button>
        </div>

        <div className='mt-5 w-full flex justify-center  mb-8'>
          <button disabled={toggler} onClick={handleSubmit} type='button' className='italic text-center bg-green-700 rounded-2xl h-12 w-3/5 hover:0 text-white text-lg' >{toggler ? "Please Wait..." : "Submit"}</button>
        </div>

      </div>
    </section>
  )



}
