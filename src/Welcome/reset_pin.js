...
function Reset() {
  ...
  const handleSubmit=(e)=>{
    ...
    axios.post(`${process.env.REACT_APP_SERVER_URL}/api/reset_pin`, data)
      .then(response=>{
        setToggler(false)
        ...
        <div className='flex justify-center items-center w-10/12 md:w-3/5' >
          <div className='bg-white w-full flex justify-center rounded-3xl text-black '>
            <form className=' flex justify-center w-full flex-col' onSubmit={(e) => handleSubmit(e)}>
              <div className='text-center h-12 rounded mt-4 font-mono font-bold text-3xl'>PIN Reset</div>

              <div className='mt-5 w-full flex justify-center '>
                <input name='pwd' placeholder='New Password' className=' w-10/12 h-16 text-center rounded-2xl placeholder:text-center  appearance-none border border-green-950 text-lg outline-none font-mono text-md font-semibold ml-6' type={state?'password':'text'} /><div className='mt-3 text-xl' onClick={handleChangeState}>{state ? <FaEye /> : <FaEyeSlash />}</div>
              </div>
              <div className='mt-5 w-full flex justify-center '>
                <input name='r_pwd' placeholder='Repeat Password' className=' w-10/12 h-16 text-center rounded-2xl placeholder:text-center  appearance-none border border-green-950 text-lg outline-none font-mono text-md font-semibold ml-6' type={state?'password':'text'} /><div className='mt-3 text-xl' onClick={handleChangeState}>{state ? <FaEye /> : <FaEyeSlash />}</div>
              </div>
